...
# Primitive Set explicitly tailored for Assessment Rules (Full 3-Variable)
pset_assessment = gp.PrimitiveSet("AssessmentRule_IR", 3) 
pset_assessment.renameArguments(ARG0='DonorAction', ARG1='DonorStanding', ARG2='RecipientStanding')
...
# =============================================================================
# 2. AGENT-BASED MODEL (Asymmetric Topology & Reputation Mechanics)
# =============================================================================
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent"):
        ...
        self.payoff = 0
        self.standing = 1 # 1 = Good Standing, 0 = Bad Standing
    ...
    def evaluate_partner(self, partner):
        ...
        # In Mode 2, BOTH Control and iGSS use the Strict Discriminator action rule
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        # 2. Institutional Assessment (Updating Standing)
        if self.control_mode:
            # HARDCODED CONTROL: The "Standing" Norm from Pseudocode
            if cooperates:
                donor.standing = 1
            else:
            ...
            # EVOLVED iGSS ASSESSMENT: Full 3-variable GP Rule
            action_val = 1 if cooperates else 0
            decision_score = self.igss_rule(action_val, donor.standing, recipient.standing)
...
def get_control_baseline(config):
    """Runs a hardcoded Standing Assessment population to establish baseline."""
    tot_control = 0
    runs = 5
    for _ in range(runs):
...
def run_evolution(config):
    ...
    print("Initializing Indirect Reciprocity [MODE 2 - Donor Variable] Module...")
    control_fitness = get_control_baseline(config)
    print(f"Control Baseline (Standing Norm) established at: {control_fitness:.2f} average payoff.")
...
if __name__ == "__main__":
    ...
    # Truth Table Tuple Mapping:
    # 8 bits -> Maps (DonorAction, DonorStanding, RecipientStanding) -> (1,1,1), (1,1,0), (1,0,1), etc...
    # Since these are the "Leading Eight" norms, they mostly ignore DonorStanding, 
    # meaning the 4-bit pattern repeats twice.
    IR_ASSESSMENT_DICT = {
        (True, True, True, True, False, False, False, False): "Image Scoring",
        (True, False, True, False, False, True, False, True): "Stern Judging",
        (True, True, True, True, False, True, False, True): "Simple Standing",
        (True, False, True, False, False, False, False, False): "Shunning",
        (True, True, True, True, True, True, True, True): "Unconditional Good",
        (False, False, False, False, False, False, False, False): "Unconditional Bad"
    }
    
    variables = ["DonorAction", "DonorStanding", "RecipientStanding"]
    ...
    identified_strategy = IR_ASSESSMENT_DICT.get(raw_tuple, f"Novel Rule: {raw_tuple}")

    run_log = {
        "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
        "Machine_ID": "Local_VSCode", 
        "Run_ID": f"IR2Donor_TEST_{datetime.now().strftime('%H%M%S')}",
        "Mode": "Indirect Reciprocity Mode 2 (Assessment w/ Donor Var)",
        **config, 
    ...
    log_results_to_csv(run_log, filename="results_IR_Mode2_Donor.csv")
    ...
    for i, config in enumerate(batches):
        ...
        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"IR2Donor_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Indirect Reciprocity Mode 2 (Assessment w/ Donor Var)",
            **config,
            "Best_Rule_Raw": str(best_rule),
            "Best_Rule_SymPy": simp_assessment,
    ...
    print("\n[✓] Batch complete! Check results_IR_Mode2_Donor.csv and the /figures directory.")