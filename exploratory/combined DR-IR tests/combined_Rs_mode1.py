...
# =============================================================================
# 2. GENETIC PROGRAMMING SETUP (DUAL-INPUT ACTION RULE)
# =============================================================================
def random_constant():
    return random.choice([-10, -1, 0, 1, 10])

# Primitive Set explicitly tailored for Institutional Competition
...
# =============================================================================
# 3. AGENT-BASED MODEL (Universal Resolver)
# =============================================================================
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent"):
        ...
        self.standing = 1     # IR Public Ledger
    ...
    def evaluate_partner(self, partner):
        ...
        # iGSS Evaluation: Reads BOTH ledgers
        in_memory_val = 1 if partner.unique_id in self.memory else 0
        decision_score = self.model.igss_rule(partner.standing, in_memory_val)
        return decision_score > 0

class CooperationModel(mesa.Model):
    def __init__(self, igss_rule, config=MODEL_CONFIG, control_type=None):
        super().__init__()
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if cooperates:
            ...
            # IR Rule: Helping restores/maintains standing
            donor.standing = 1 
        else:
            # DR Rule: Defection adds you to private memory
            recipient.memory.add(donor.unique_id)
            # DR Rule: Forgiveness if this was a justified retaliation
            if recipient.unique_id in donor.memory:
                donor.memory.remove(recipient.unique_id)
                
            # IR Rule: Defecting against a Good agent destroys public standing
...
def get_control_baseline(model_config, control_type):
    """Runs a hardcoded baseline for either DR (Control-TFT) or IR (Control-Standing)."""
    tot_fitness = 0
    runs = 5
    for _ in range(runs):
        m = CooperationModel(igss_rule=None, config=model_config, control_type=control_type)
        for _ in range(model_config["NUM_ROUNDS"]): m.step()
...
def run_evolution():
    ...
    print("Initializing Combined Baseline Module (Institutional Competition)...")
    ...
    print(f"Control Baseline (DR - Tit-for-Tat): {dr_control_fitness:.1f}")
...
def plot_dashboard(best_ind, history, model_config, evo_config, dr_fit, ir_fit):
    ...
    report_text = (
        f"\n{'='*60}\n"
        f"      COMBINED [MODE 1] REPORT (Institutional Competition)\n"
        f"{'='*60}\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
        ...
        f"Control Baseline (DR - Tit-for-Tat): {dr_fit:.1f}\n"
    ...
    ax_plot.set_title('Institutional Competition: Evolving the Action Rule')
