...
# =============================================================================
# 2. GENETIC PROGRAMMING SETUP (STRICT 2-VARIABLE JUSTICE SYSTEM)
# =============================================================================
def random_constant():
    return random.choice([-10, -1, 0, 1, 10])

# Primitive Set restricted strictly to Action and Recipient's Reputation
...
pset_assessment.addEphemeralConstant("rand_const_act", random_constant)
...
# =============================================================================
# 3. AGENT-BASED MODEL (Asymmetric Topology & Reputation Mechanics)
# =============================================================================
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent"):
        ...
        self.payoff = 0
        self.standing = 0 # reversed ## 1 = Good Standing, 0 = Bad Standing
    ...
    def evaluate_partner(self, partner):
        ...
        # --- MODIFICATION START ---
        if self.agent_type == "iGSS-Agent":
            # INVERTED ACTION RULE: Cooperate only if partner has BAD (0) standing.
            return partner.standing == 0
        
        # Control-Agent keeps the standard Strict Discriminator (cooperate if Good standing)
...
# =============================================================================
# 4. EVOLUTIONARY ENGINE (With Elitism)
# =============================================================================
...
def run_evolution():
    ...
    print("Initializing Indirect Reciprocity [MODE 2 - Inverted Action Rule]...")
...
def plot_deap_tree(individual):
    ...
    ax_tree.set_title("GP Assessment Rule (Inverted Action Rule)", fontsize=14)
    ax_tree.axis("off")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*50}\n"
        f"      INDIRECT RECIPROCITY [MODE 2 - INVERTED] REPORT\n"
        f"{'='*50}\n"
        f"--- BEST EVOLVED ASSESSMENT RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
        ...
        f"Control Baseline (Standing Norm): {control_fit:.1f}\n"
    ...
    ax_plot.axhline(y=control_fit, color='black', linestyle=':', label=f'Control Baseline (Standing Norm): {control_fit:.1f}')
    ax_plot.axhline(y=theoretical_max, color='purple', alpha=0.3, label=f'Maximum Theoretical Fitness: {theoretical_max}')
    
    ax_plot.set_title('Indirect Reciprocity [Mode 2 - Inverted Action]: Assessment Norms')
