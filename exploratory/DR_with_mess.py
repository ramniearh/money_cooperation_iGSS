...
# =============================================================================
# 2. GENETIC PROGRAMMING SETUP (THE NEW PRIMITIVES)
# =============================================================================
def random_constant():
    return random.choice([-1, 1])

# Primitives expanded to include Junk and the Correlating Device (Clock)
pset_action = gp.PrimitiveSet("ActionRule_DR", 4) 
pset_action.renameArguments(ARG0='My_Type', ARG1='Clock', ARG2='PartnerInMemory', ARG3='Junk')
...
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent", my_type=1):
        super().__init__(model) 
        self.agent_type = agent_type
        self.my_type = my_type # Identity for symmetry breaking
        self.payoff = 0
        self.memory = set() 
    ...
    def evaluate_partner(self, partner):
        ...
        # iGSS Evaluation with all 4 signals
        in_memory_signal = 1 if partner.unique_id in self.memory else -1
        junk_signal = random.choice([-1, 1])
        
        decision_score = self.model.igss_rule(self.my_type, self.model.clock, in_memory_signal, junk_signal)
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_rule, config=MODEL_CONFIG, control_mode=False):
        ...
        self.clock = 1 # Global Correlating Device
        
        target_agent = "Control-TFT" if control_mode else "iGSS-Agent"
        
        # Assign half of the agents as Type 1, half as Type -1
        for i in range(config["NUM_IGSS"]): CoopAgent(self, target_agent, my_type=1 if i%2==0 else -1)
        for i in range(config["NUM_UC"]): CoopAgent(self, "Unconditional Cooperator", my_type=1 if i%2==0 else -1)
        for i in range(config["NUM_D"]): CoopAgent(self, "Defector", my_type=1 if i%2==0 else -1)

    def step(self):
        self.clock *= -1 # Flip the clock every round
...
def run_evolution():
    ...
    print("Initializing The Correlating Device / Junk DNA Test...")
...
def simplify_rule(gp_string):
    mapping = {
        ...
        'sub': lambda x, y: x - y, 
        'mul': lambda x, y: x * y,
        'My_Type': sp.Symbol('Type'),
        'Clock': sp.Symbol('Clock'),
        'PartnerInMemory': sp.Symbol('Mem'),
        'Junk': sp.Symbol('Junk')
    }
    try: 
...
# [hierarchy_pos, plot_deap_tree, and plot_dashboard remain identical to your DR_mode1.py]
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*50}\n"
        f"      CORRELATING DEVICE vs MEMORY TEST\n"
        f"{'='*50}\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
        ...
        f"Control Baseline (Tit-for-Tat): {control_fit:.1f}\n"
    ...
    ax_plot.set_title('Rule Discovery: Memory vs. Global Clock')
