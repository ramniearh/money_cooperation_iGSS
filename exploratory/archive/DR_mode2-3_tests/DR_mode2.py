...
# =============================================================================
# 1. GLOBAL CONFIGURATION (High Pressure for DR)
# =============================================================================
MODEL_CONFIG = {
    ...
    "NUM_IGSS": 10,  
    "NUM_UC": 10,    
    "NUM_D": 20,     # High defectors to test memory utility
    "NUM_ROUNDS": 100 # Extended rounds to make getting suckered expensive
}
...
# =============================================================================
# 2. SINGLE-TREE GP SETUP (Memory Update Rule Only)
# =============================================================================
...
# TREE: Memory Rule (Inputs: PartnerAction, PartnerWasInMemory)
# Output: > 0 means Add/Keep in Memory (Grudge), <= 0 means Remove (Forgive)
pset_memory = gp.PrimitiveSet("MemoryRule", 2)
...
toolbox.register("individual", tools.initIterate, creator.Individual, toolbox.expr_memory)
toolbox.register("population", tools.initRepeat, list, toolbox.individual)

toolbox.register("mate", gp.cxOnePoint)
toolbox.register("mutate", gp.mutUniform, expr=toolbox.expr_memory, pset=pset_memory)
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # MODE 2 DR: Hardcoded Action Rule (Strict Defensive)
        # Cooperate if partner is NOT in memory; Defect if they ARE in memory.
        return partner.unique_id not in self.memory

class CooperationModel(mesa.Model):
    def __init__(self, memory_rule, config=MODEL_CONFIG):
        super().__init__()
    ...
    def resolve(self, helper, recipient, cooperates):
        ...
        # Evolved Memory logic (The "Assessment")
...
def evaluate_rule(individual, model_config, evo_config):
    func_mem = gp.compile(expr=individual, pset=pset_memory)
    
    total_fitness = 0
    for _ in range(3): 
        m = CooperationModel(memory_rule=func_mem, config=model_config)
...
def run_evolution():
    ...
    print("Initializing Mode 2 DR (Fixed Action, Evolved Memory)...")
...
# =============================================================================
def plot_dashboard(best_ind, history, model_config, title="Mode 2 DR: Evolving Memory/Forgiveness"):
    fig, (ax_plot, ax_text) = plt.subplots(1, 2, figsize=(15, 6), gridspec_kw={'width_ratios': [1.8, 1.2]})
    ...
    ax_plot.plot(max_efficiency, label='Max Efficiency', color='forestgreen', linewidth=2)
    ax_plot.plot(avg_efficiency, label='Avg Efficiency', color='limegreen', linestyle='--')
    ...
    report_text = (
        ...
        f"ACTION RULE:\n  HARDCODED: Cooperate if NOT in memory.\n\n"
        f"MEMORY RULE:\n  {str(best_ind)}\n\n"
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("      MODE 2 DR FINAL LAB REPORT")
    print("="*50)
    print("--- EVOLVED MEMORY RULE ---")
    print(f"Logic: {best_ind}")
    print("\n[Legend: ARG0=PartnerAction | ARG1=PartnerWasInMemory]")
