...
MODEL_CONFIG = {
    ...
    "COST": 1,
    "NUM_IGSS": 20, 
    "NUM_UC": 0,    # DORMANT ECOLOGY
    "NUM_D": 0,     # DORMANT ECOLOGY
    "NUM_ROUNDS": 50,
...
# =============================================================================
# 3. AGENT-BASED MODEL (Clamped Consensus Physics)
# =============================================================================
class CoopAgent(mesa.Agent):
    ...
    def evaluate_donor(self):
        if self.agent_type == "Unconditional Cooperator": return 0 
        if self.agent_type == "Defector": return None # Hard assumption: Defectors abort institutional consensus when asked to give.
    ...
    def evaluate_recipient(self):
        if self.agent_type == "Unconditional Cooperator": return 0 
        if self.agent_type == "Defector": return 0 # Defectors happily accept free logic
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_donor_rule, igss_recipient_rule, config=MODEL_CONFIG):
        ...
        agents_list = []
        for _ in range(config["NUM_IGSS"]): agents_list.append(CoopAgent(self, "iGSS-Agent"))
        ...
        economic_agents = [a for a in agents_list if a.agent_type == "iGSS-Agent"]
    ...
    def step(self):
        ...
        for donor in agents:
            ...
            donor_val = donor.evaluate_donor()
            recipient_val = recipient.evaluate_recipient()
            
            if donor_val is None or recipient_val is None:
                continue
            
            if donor_val + recipient_val == 0:
                self.resolve_transaction(donor, recipient, donor_val)
            
    def resolve_transaction(self, donor, recipient, transfer_val):
        cares_about_money = donor.agent_type == "iGSS-Agent" and recipient.agent_type == "iGSS-Agent"
        
        valid_trade = True
        if cares_about_money:
            # Hard assumption: Strict Fiat constraint. No negative balances allowed.
            if transfer_val == 1 and recipient.token < 1: valid_trade = False
            if transfer_val == -1 and donor.token < 1: valid_trade = False

        if valid_trade:
            donor.payoff -= self.cost
            recipient.payoff += self.benefit
            
            if cares_about_money and transfer_val != 0:
...
def run_evolution():
    ...
    print("Initializing CLAMPED CONSENSUS [MODE 9] Module...")
...
def plot_dashboard(best_ind, history, model_config, evo_config):
    ...
    report_text = (
        f"\n{'='*70}\n"
        f"      CLAMPED CONSENSUS [MODE 9] REPORT\n"
        f"{'='*70}\n"
        f"--- ECONOMIC PHYSICS (ASSUMPTIONS) ---\n"
        f"Defector Constraint: Donors hard-abort (return None)\n"
        ...
        f"Tournament Size:     {evo_config['TOURNAMENT_SIZE']} (Selection Pressure)\n"
        f"Mutation Rate:       {evo_config['MUTATION_RATE']}\n"
        f"Crossover Rate:      {evo_config['CROSSOVER_RATE']}\n"
    ...
    ax_plot.set_title('Clamped Consensus [Mode 9]')
