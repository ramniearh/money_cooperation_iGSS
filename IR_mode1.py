...
# Primitive Set explicitly tailored for Indirect Reciprocity (Mode 1)
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        if cooperates:
            ...
            # The Standing Norm: Defecting against a "Good" agent destroys your standing
...
def run_evolution(config):
    ...
    print(f"Control Baseline (Strict Standing) established at: {control_fitness:.2f} average payoff.")
...
if __name__ == "__main__":
    ...
    # Truth Table Mapping specific to IR Mode 1.
    # Note on Tuples: (Output when Standing is Good [1], Output when Standing is Bad [0])
    # True = Cooperate, False = Defect
    IR_ACTION_DICT = {
        (True, False): "Strict Discriminator", # Cooperate if Good, Defect if Bad
        (True, True): "Unconditional Cooperator (ALL-C)",
        (False, False): "Unconditional Defector (ALL-D)",
        (False, True): "Anti-Discriminator"    # Defect if Good, Cooperate if Bad
    }
    ...
    for i, config in enumerate(batches):
        ...
        # 2. Parse the Math & Identify Strategy
        simp_action = str(simplify_rule(str(best_rule), variables))
        raw_tuple = evaluate_truth_table(simp_action, variables)
        identified_strategy = IR_ACTION_DICT.get(raw_tuple, f"Novel Rule: {raw_tuple}")
    ...
    print("\n[✓] Batch complete! Check results_IR_Mode1.csv and the /figures directory.")