...
# Primitive Set explicitly tailored for Assessment Rules (Strict 2-Variable)
pset_assessment = gp.PrimitiveSet("AssessmentRule_IR_Strict", 2) 
pset_assessment.renameArguments(ARG0='DonorAction', ARG1='RecipientStanding')
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if self.control_mode:
            ...
            # EVOLVED iGSS ASSESSMENT: Strict 2-variable GP Rule
            action_val = 1 if cooperates else 0
            decision_score = self.igss_rule(action_val, recipient.standing)
...
def run_evolution(config):
    ...
    print("Initializing Indirect Reciprocity [MODE 2 - No Donor Variable] Module...")
...
if __name__ == "__main__":
    ...
    # 4 bits -> (C against Good, C against Bad, D against Good, D against Bad)
    IR_ASSESSMENT_DICT = {
        (True, True, False, False): "Image Scoring (binary)",     # Ignore recipient standing; action only
        (True, False, False, True): "Stern Judging",     # C|G is Good, D|B is Good. Rest Bad.
        (True, True, False, True): "Simple Standing",    # C|G is Good, C|B is Good, D|B is Good.
        (True, False, False, False): "Shunning",         # Only C|G is Good. Rest Bad.
        (True, True, True, True): "Unconditional Good",
        (False, False, False, False): "Unconditional Bad"
    }
    
    variables = ["DonorAction", "RecipientStanding"]
    ...
    run_log = {
        ...
        "Run_ID": f"IR2NoD_TEST_{datetime.now().strftime('%H%M%S')}",
        "Mode": "Indirect Reciprocity Mode 2 (Assessment w/ No Donor Var)",
        **config, 
    ...
    log_results_to_csv(run_log, filename="results_IR_Mode2.csv")
    ...
    for i, config in enumerate(batches):
        ...
        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"IR2NoD_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Indirect Reciprocity Mode 2 (Assessment w/ No Donor Var)",
            **config,
            "Best_Rule_Raw": str(best_rule),
            "Best_Rule_SymPy": simp_assessment,
    ...
    print("\n[✓] Batch complete! Check results_IR_Mode2.csv and the /figures directory.")