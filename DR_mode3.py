...
# --- Tree 1: Action Rule (Behavior based on Memory) ---
...
# --- Tree 2: Assessment Rule (Updating the Memory Ledger) ---
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        decision_score = self.model.igss_action_rule(in_memory_val)
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if self.control_mode:
            # HARDCODED CONTROL: Strict Tit-for-Tat
            if not cooperates:
...
def run_evolution(config):
    ...
    print("Initializing Direct Reciprocity [MODE 3 - Co-Evolution Search]...")
...
if __name__ == "__main__":
    ...
    DR_ACTION_DICT = {
        (False, True): "Tit-for-Tat",         
        (True, True): "Unconditional Cooperator (ALL-C)",
        (False, False): "Unconditional Defector (ALL-D)",
        (True, False): "Anti-Tit-for-Tat"     
    }
    
    DR_MEMORY_DICT = {
        (False, False, True, True): "Strict Ledger (TFT Base)", 
        (True, True, True, True): "Paranoid (Remember Everyone)",
        (False, False, False, False): "Amnesia (Remember No One)",
        (True, False, True, True): "Unforgiving (Grim Trigger)", 
        (True, False, True, False): "Sticky Memory (Ignore actions entirely)" 
    }
    
    action_vars = ["PartnerInMemory"]
    assess_vars = ["DonorAction", "DonorInMemory"]
    ...
    identified_action = DR_ACTION_DICT.get(act_tuple, f"Novel Act: {act_tuple}")
    ...
    identified_assessment = DR_MEMORY_DICT.get(ass_tuple, f"Novel Mem: {ass_tuple}")

    run_log = {
        "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
        "Machine_ID": "Local_VSCode", 
        "Run_ID": f"DR3_TEST_{datetime.now().strftime('%H%M%S')}",
        "Mode": "Direct Reciprocity Mode 3 (Co-Evolution)",
        **config, 
    ...
    log_results_to_csv(run_log, filename="results_DR_Mode3.csv")
    ...
    for i, config in enumerate(batches):
        ...
        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"DR3_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Direct Reciprocity Mode 3 (Co-Evolution)",
            **config,
            "Best_Act_Raw": str(best_rule[0]),
            "Best_Act_SymPy": simp_action,
    ...
    print("\n[✓] Batch complete! Check results_DR_Mode3.csv and the /figures directory.")