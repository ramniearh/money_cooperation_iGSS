...
# Boosted to handle 2-Tree Co-Evolution
EVO_CONFIG = {
    "POP_SIZE": 100,        
    "MAX_GENS": 100,         
    "PARSIMONY_TAX": 0.1
...
# --- Tree 2: Universal Assessment Rule ---
pset_ass = gp.PrimitiveSet("AssessmentRule_Combined", 3) 
pset_ass.renameArguments(ARG0='DonorAction', ARG1='RecipientStanding', ARG2='DonorInMemory')
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if self.control_type == "Control-TFT":
            ...
            # iGSS EVOLVED UNIVERSAL ASSESSMENT RULE
            action_val = 1 if cooperates else 0
            donor_in_mem = 1 if donor.unique_id in recipient.memory else 0
            
            decision_score = self.rule_ass(action_val, recipient.standing, donor_in_mem)
...
def run_evolution():
    ...
    print("Initializing Combined [MODE 3] (Disciplined 2-Tree Co-Evolution)...")
...
def parse_sympy(gp_string, rule_type):
    if rule_type == "ACT":
        ...
        mapping = {'add': lambda x,y: x+y, 'sub': lambda x,y: x-y, 'mul': lambda x,y: x*y, 'DonorAction': sp.Symbol('DonorAction'), 'RecipientStanding': sp.Symbol('RecipientStanding'), 'DonorInMemory': sp.Symbol('DonorInMemory')}
