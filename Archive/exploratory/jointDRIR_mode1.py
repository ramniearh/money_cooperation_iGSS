...
# Primitive Set explicitly tailored for Combined DR & IR
...
# =============================================================================
# 3. AGENT-BASED MODEL (Asymmetric Topology & Dual Mechanics)
# =============================================================================
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_rule, config=MODEL_CONFIG, control_mode=False):
        ...
        target_agent = "Control-Hybrid" if control_mode else "iGSS-Agent"
    ...
    def resolve(self, donor, recipient, cooperates):
        if cooperates:
            ...
            # 1. DR Update: Recipient remembers the defector
            recipient.memory.add(donor.unique_id)
            
            # DR Forgiveness Execution
            if recipient.unique_id in donor.memory:
                donor.memory.remove(recipient.unique_id)
                
            # 2. IR Update: Defecting against a "Good" agent destroys your standing
            if recipient.standing == 1:
                donor.standing = 0

    def get_fitness_by_type(self):
        fitness_data = {}
        for a_type in ["iGSS-Agent", "Control-Hybrid", "Unconditional Cooperator", "Defector"]:
...
def get_control_baseline(model_config):
    """Runs a strict Hybrid population to establish the theoretical optimal baseline."""
    tot_hybrid = 0
    runs = 5
    for _ in range(runs):
        m = CooperationModel(igss_rule=None, config=model_config, control_mode=True)
        for _ in range(model_config["NUM_ROUNDS"]): m.step()
        tot_hybrid += m.get_fitness_by_type()["Control-Hybrid"]
    return tot_hybrid / runs
...
def run_evolution():
    ...
    print("Initializing Combined DR & IR [MODE 1] Module...")
    control_fitness = get_control_baseline(MODEL_CONFIG)
    print(f"Control Baseline (Hybrid AND Logic) established at: {control_fitness:.2f} average payoff.")
...
def simplify_rule(gp_string):
    mapping = {
        ...
        'sub': lambda x, y: x - y, 
        'mul': lambda x, y: x * y,
        'PartnerInMemory': sp.Symbol('PartnerInMemory'),
...
def plot_deap_tree(individual):
    ...
    ax_tree.set_title("Combined Action Rule AST", fontsize=14)
    ax_tree.axis("off")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*50}\n"
        f"      COMBINED DR & IR [MODE 1] REPORT\n"
        f"{'='*50}\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
    ...
    ax_plot.set_title('Combined DR & IR [Mode 1]: Evolutionary Performance')
