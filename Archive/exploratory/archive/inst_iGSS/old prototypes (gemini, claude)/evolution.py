...
# FIX #1: We use a proper function instead of a lambda for the constant
def random_constant():
...
def evaluate_rule_in_mesa(individual):
    ...
    total_fitness = 0
    for _ in range(3):
        m = ExchangeModel(30, rule=rule_func)
        for _ in range(10):
            m.step()
...
if __name__ == "__main__":
    ...
    for gen in range(1, 6): # Changed to 1 to 6 for clearer printing
        # Select the best and clone them
        offspring = toolbox.select(pop, len(pop))
        offspring = list(map(toolbox.clone, offspring))
        
        # Mutate and cross over (Mating)
        for child1, child2 in zip(offspring[::2], offspring[1::2]):
            if random.random() < 0.5:
                toolbox.mate(child1, child2)
                # We erase the parents' fitness because the children are new
        ...
        for mutant in offspring:
            if random.random() < 0.2:
                ...
                # Erase fitness for the mutant
                del mutant.fitness.values
                
        # FIX #2: Evaluate the NEW children/mutants before scoring!
        invalid_ind = [ind for ind in offspring if not ind.fitness.valid]
        fitnesses = map(toolbox.evaluate, invalid_ind)
        for ind, fit in zip(invalid_ind, fitnesses):
            ind.fitness.values = fit
                
        # Replace the old population with the new, smarter one
        pop[:] = offspring
