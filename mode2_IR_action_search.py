...
# =============================================================================
# 2. SINGLE-TREE GP SETUP (Assessment Rule Only)
# =============================================================================
...
toolbox.register("individual", tools.initIterate, creator.Individual, toolbox.expr_assess)
toolbox.register("population", tools.initRepeat, list, toolbox.individual)

toolbox.register("mate", gp.cxOnePoint)
toolbox.register("mutate", gp.mutUniform, expr=toolbox.expr_assess, pset=pset_assess)
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # MODE 2: Hardcoded Action Rule (Strict Discriminator)
        # Cooperate if partner is Good, Defect if partner is Bad
        return partner.standing > 0

class CooperationModel(mesa.Model):
    def __init__(self, assessment_rule, config=MODEL_CONFIG):
        super().__init__()
...
def evaluate_rule(individual, model_config, evo_config):
    func_assess = gp.compile(expr=individual, pset=pset_assess)
    
    total_fitness = 0
    for _ in range(3): 
        m = CooperationModel(assessment_rule=func_assess, config=model_config)
...
def run_evolution():
    ...
    print("Initializing Mode 2 (Fixed Action, Evolved Assessment)...")
...
# =============================================================================
def plot_dashboard(best_ind, history, model_config, title="Mode 2: Evolving Assessment Rules"):
    ...
    report_text = (
        ...
        f"ACTION RULE:\n  HARDCODED: Cooperate if Standing > 0\n\n"
        f"ASSESSMENT RULE:\n  {str(best_ind)}\n\n"
