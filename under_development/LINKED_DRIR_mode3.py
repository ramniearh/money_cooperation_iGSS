...
# =============================================================================
# 1. GENETIC PROGRAMMING SETUP (CO-EVOLUTION)
# =============================================================================
def random_constant():
    return random.choice([-10, -1, 0, 1, 10])

# --- Tree 1: Action Rule (2 Variables) ---
...
# --- Tree 2: Assessment Rule (3 Variables) ---
pset_assessment = gp.PrimitiveSet("AssessmentRule_Combined", 3) 
pset_assessment.renameArguments(ARG0='DonorAction', ARG1='DonorInMemory', ARG2='RecipientStanding')
...
if not hasattr(creator, "FitnessMax"):
    ...
    # The Individual is now a LIST containing two separate trees
...
def cxTwoTrees(ind1, ind2):
    # Flip a coin: crossover the Action tree (0) OR the Assessment tree (1)
...
def mutTwoTrees(individual):
    # Flip a coin: mutate the Action tree (0) OR the Assessment tree (1)
    if random.random() < 0.5:
...
# =============================================================================
# 2. AGENT-BASED MODEL (Unified Linked Topology)
# =============================================================================
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent"):
        ...
        self.payoff = 0
        self.memory = set()   # DR Private Ledger
        self.standing = 1     # IR Public Reputation
    ...
    def evaluate_partner(self, partner):
        ...
        # Hardcoded Control Baseline Action: "The Skeptic"
        ...
        decision_score = self.model.igss_action_rule(in_memory_signal, partner.standing)
        return decision_score > 0

class CooperationModel(mesa.Model):
    def __init__(self, igss_action_rule, igss_assessment_rule, config, control_mode=False):
        super().__init__()
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if self.control_mode:
            # HARDCODED CONTROL JUSTICE (From Mode 1)
            if cooperates:
                donor.standing = 1 
            ...
            in_memory_val = 1 if donor.unique_id in recipient.memory else 0
            
            decision_score = self.igss_assessment_rule(action_val, in_memory_val, recipient.standing)
...
def evaluate_rules(individual, config):
    ...
    # Parsimony Tax is calculated against the sum of both trees
...
def get_control_baseline(config):
    ...
    runs = 5
    for _ in range(runs):
        m = CooperationModel(igss_action_rule=None, igss_assessment_rule=None, config=config, control_mode=True)
        for _ in range(config["NUM_ROUNDS"]): m.step()
...
def run_evolution(config):
    ...
    print("Initializing Combined Ecology [MODE 3 - Full Co-Evolution Search]...")
...
if __name__ == "__main__":
    ...
    # 4-bit Action Dictionaries 
    COMBINED_ACTION_DICT = {
        (True, True, True, True): "Unconditional Cooperator (ALL-C)",
        (False, False, False, False): "Unconditional Defector (ALL-D)",
        (True, False, True, False): "Values Public Gossip (Public Discriminator)", 
        (False, False, True, True): "Values Personal Experience (Private Discriminator)", 
        (False, False, True, False): "The Skeptic", 
        (True, False, True, True): "Benefit of the Doubt" 
    }
    
    # 8-bit Assessment Dictionaries
    COMBINED_ASSESSMENT_DICT = {
        (True, True, True, True, False, False, False, False): "Pure Image Scoring",
        (True, True, True, True, True, True, True, True): "Unconditional Forgiveness",
        (False, False, False, False, False, False, False, False): "Unconditional Condemnation"
    }
    
    action_vars = ["PartnerInMemory", "PartnerStanding"]
    assess_vars = ["DonorAction", "DonorInMemory", "RecipientStanding"]
    ...
    for i, config in enumerate(batches):
        ...
        identified_action = COMBINED_ACTION_DICT.get(act_tuple, f"Novel Act: {act_tuple}")
        ...
        identified_assessment = COMBINED_ASSESSMENT_DICT.get(ass_tuple, f"Novel Ass: {ass_tuple}")

        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"Combined3_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Combined Ecology Mode 3 (Co-Evolution)",
            **config,
        ...
        log_results_to_csv(run_log, filename="results_Combined_Mode3.csv")
        generate_dashboard(best_rule, history, run_log, save_only=True)
        
    print("\n[✓] Batch complete! Check results_Combined_Mode3.csv and the /figures directory.")