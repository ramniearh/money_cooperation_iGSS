#### this SHOULD match the original r&s paper. but needs validation
# 
import operator
import random
import numpy as np
...
MODEL_CONFIG = {
    ...
    "INITIAL_ENDOWMENT": 1,        
    "ENDOWMENT_FRACTION": 0.5      # 0.5 = 50% Liquidity
}
...
# =============================================================================
# 3. AGENT-BASED MODEL (Closed Economy Physics)
# =============================================================================
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_rule, config=MODEL_CONFIG, control_mode=False):
        ...
        # Only the target economic agents are allowed to receive starting funds
    ...
    def resolve(self, donor, recipient, cooperates):
        if cooperates:
            ...
            # 2. The Institutional Settlement (Matches Figure 1 of your paper)
            # Token only moves if the DONOR'S strategy explicitly demands it.
...
def run_evolution():
    ...
    print("Initializing CLOSED ECONOMY MONEY [MODE 1.6] Module...")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*60}\n"
        f"      CLOSED ECONOMY MONEY [MODE 1.6] REPORT\n"
        f"{'='*60}\n"
        f"--- CONFIGURATIONS ---\n"
        f"Random Seed: {SEED}\n"
        f"Populations: {model_config['NUM_IGSS']} iGSS | {model_config['NUM_UC']} UC | {model_config['NUM_D']} Defectors\n"
        f"Liquidity (Fiat Genesis): {model_config['INITIAL_ENDOWMENT']} tokens to {int(model_config['ENDOWMENT_FRACTION'] * 100)}% of money-users\n\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
    ...
    ax_plot.set_title('Closed Economy Money [Mode 1.6]: Evolutionary Dynamics')
