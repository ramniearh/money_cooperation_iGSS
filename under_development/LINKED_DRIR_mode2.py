...
# Primitive Set tailored for Combined DR+IR Assessment Search
# It now takes THREE variables to judge an interaction.
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # MODE 2 ACTION RULE: Hardcoded to "The Skeptic" for both Control and iGSS agents
        # Requires BOTH a clean public record and a clean private memory to cooperate
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        # 2. Institutional Assessment (The Justice System)
        if self.control_mode:
            # HARDCODED CONTROL JUSTICE
            if cooperates:
                donor.standing = 1 
            ...
            # EVOLVED iGSS ASSESSMENT: Uses the 3-variable Tree
            action_val = 1 if cooperates else 0
            in_memory_val = 1 if donor.unique_id in recipient.memory else 0
            
            decision_score = self.igss_rule(action_val, in_memory_val, recipient.standing)
...
def run_evolution(config):
    ...
    print("Initializing Combined Ecology [MODE 2 - Unified Assessment Search]...")
...
if __name__ == "__main__":
    ...
    # 8-bit Truth Table Mapping
    # Logic Map evaluates 8 permutations of (Action, InMem, RecipStanding)
    # Output: True = Forgive/Good Standing, False = Condemn/Bad Standing
    # Note: Due to 256 possible strategies, only the most famous archetypes are mapped.
    COMBINED_ASSESSMENT_DICT = {
        # Pure Image Scoring: Forgive if C, Condemn if D (ignores memory and recipient standing entirely)
        (True, True, True, True, False, False, False, False): "Pure Image Scoring",
        
        # Pure Private Ledger: Forgive if C, Condemn if D (ignores recipient standing, relies on action only)
        # Note: In this 3-var setup, pure private evaluation looks mathematically identical to Image Scoring 
        # because the rule simply anchors on the DonorAction boolean.
    ...
    variables = ["DonorAction", "DonorInMemory", "RecipientStanding"]
    ...
    for i, config in enumerate(batches):
        ...
        identified_strategy = COMBINED_ASSESSMENT_DICT.get(raw_tuple, f"Novel Rule: {raw_tuple}")

        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"Combined2_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Combined Ecology Mode 2 (Assessment Search)",
            **config,
        ...
        log_results_to_csv(run_log, filename="results_Combined_Mode2.csv")
        generate_dashboard(best_rule, history, run_log, save_only=True)
        
    print("\n[✓] Batch complete! Check results_Combined_Mode2.csv and the /figures directory.")