"""
How to use this with your updated Combined_Mode3.py script:
To ensure this picks up your three baselines perfectly, just make sure that when you build the run_log dictionary inside your Mode 3 execution block, you pass those three specific keys:

run_log = {
    # ... all the other stuff ...
    "Baseline_Skeptic": round(control_fit_skeptic, 2),
    "Baseline_TFT": round(control_fit_tft, 2),
    "Baseline_Standing": round(control_fit_standing, 2)
}
The utilities.py file will dynamically detect those keys, print them in a neat stack in your terminal report, plot them as three distinct colored horizontal lines on your matplotlib chart, and automatically add them as new columns to your CSV output.
"""
...
def generate_dashboard(best_ind, history, run_log, save_only=True):
    ...
    # -------------------------------------------------------------------------
    # MULTI-BASELINE REPORTING LOGIC
    # -------------------------------------------------------------------------
    if "Control_Baseline" in run_log:
        report_text += f"Control Baseline: {run_log.get('Control_Baseline', 'N/A')}\n"
    
    # New Joint Mode Baselines
    if "Baseline_Skeptic" in run_log:
        report_text += f"  > Baseline (The Skeptic): {run_log.get('Baseline_Skeptic', 'N/A')}\n"
    if "Baseline_TFT" in run_log:
        report_text += f"  > Baseline (Tit-for-Tat): {run_log.get('Baseline_TFT', 'N/A')}\n"
    if "Baseline_Standing" in run_log:
        report_text += f"  > Baseline (Standing):    {run_log.get('Baseline_Standing', 'N/A')}\n"

    # Old Mixed Ecology Baselines (Kept for backward compatibility)
    ...
    report_text += f"\niGSS Max Payoff: {run_log.get('iGSS_Max_Fitness', 'N/A')}\n"
    ...
    if fossil_dict:
        ...
        if len(sorted_gens) <= 5:
            ...
            display_gens = sorted(list(set(display_gens))) 
    ...
    # -------------------------------------------------------------------------
    # MULTI-BASELINE PLOTTING LOGIC
    # -------------------------------------------------------------------------
    # Backward compatibility for old scripts
    if "Control_Baseline" in run_log and run_log["Control_Baseline"] != 'N/A':
        ax_plot.axhline(y=float(run_log['Control_Baseline']), color='black', linestyle=':', label=f'Control Baseline: {run_log["Control_Baseline"]}')
        
    # New Joint Mode Baselines
    if "Baseline_Skeptic" in run_log and run_log["Baseline_Skeptic"] != 'N/A':
        ax_plot.axhline(y=float(run_log['Baseline_Skeptic']), color='black', linestyle=':', label=f'Skeptic: {run_log["Baseline_Skeptic"]}')
    if "Baseline_TFT" in run_log and run_log["Baseline_TFT"] != 'N/A':
        ax_plot.axhline(y=float(run_log['Baseline_TFT']), color='orange', alpha=0.7, linestyle='--', label=f'TFT: {run_log["Baseline_TFT"]}')
    if "Baseline_Standing" in run_log and run_log["Baseline_Standing"] != 'N/A':
        ax_plot.axhline(y=float(run_log['Baseline_Standing']), color='purple', alpha=0.7, linestyle='--', label=f'Standing: {run_log["Baseline_Standing"]}')
        
    # Old Mixed Ecology Baselines
    if "Control_DR_Score" in run_log:
