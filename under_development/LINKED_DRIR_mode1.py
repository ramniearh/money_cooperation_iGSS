...
# Primitive Set explicitly tailored for Combined DR+IR Action Search
# It now takes TWO variables to make an action decision.
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # Hardcoded Control Baseline: "The Skeptic"
        # Requires BOTH a clean public record and a clean private memory
        ...
        if self.agent_type == "Control-Skeptic":
            return (partner.standing == 1) and (partner.unique_id not in self.memory)
        
            
        # EVOLVED iGSS ACTION: Uses the 2-variable Tree
        in_memory_signal = 1 if partner.unique_id in self.memory else 0
        decision_score = self.model.igss_rule(in_memory_signal, partner.standing)
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_rule, config, control_mode=False):
        ...
        target_agent = "Control-Skeptic" if control_mode else "iGSS-Agent"
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if cooperates:
            ...
            # UNIFIED HARDCODED JUSTICE: Forgiveness & Restoration
            donor.standing = 1 
            recipient.memory.discard(donor.unique_id)
        else:
            # UNIFIED HARDCODED JUSTICE: Condemnation & Grudges
            recipient.memory.add(donor.unique_id)
            
            # The Standing Norm (only ruin standing if they hurt a Good agent)
            if recipient.standing == 1:
                donor.standing = 0

    def get_fitness_by_type(self):
        fitness_data = {}
        for a_type in ["iGSS-Agent", "Control-Skeptic", "Unconditional Cooperator", "Defector"]:
...
def get_control_baseline(config):
    ...
    for _ in range(runs):
        ...
        for _ in range(config["NUM_ROUNDS"]): m.step()
        tot_control += m.get_fitness_by_type()["Control-Skeptic"]
...
def run_evolution(config):
    ...
    print("Initializing Combined Ecology [MODE 1 - Unified Action Search]...")
    control_fitness = get_control_baseline(config)
    print(f"Control Baseline (The Skeptic) established at: {control_fitness:.2f} average payoff.")
...
if __name__ == "__main__":
    ...
    # 4-bit Truth Table Mapping
    # Logic Map: (InMem=True/GoodStand, InMem=True/BadStand, InMem=False/GoodStand, InMem=False/BadStand)
    ...
    COMBINED_ACTION_DICT = {
        ...
        # Ignores memory entirely, trusts standing
        (True, False, True, False): "Values Public Gossip (Public Discriminator)", 
        
        # Ignores standing entirely, trusts personal memory
        (False, False, True, True): "Values Personal Experience (Private Discriminator)", 
        
        # Requires BOTH to be good to cooperate
        (False, False, True, False): "The Skeptic", 
        
        # Will cooperate if AT LEAST ONE is good (Forgiving)
        (True, False, True, True): "Benefit of the Doubt" 
    }
    
    variables = ["PartnerInMemory", "PartnerStanding"]
    ...
    identified_strategy = COMBINED_ACTION_DICT.get(raw_tuple, f"Novel Rule: {raw_tuple}")

    run_log = {
        "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
        "Machine_ID": "Local_VSCode", 
        "Run_ID": f"Combined1_TEST_{datetime.now().strftime('%H%M%S')}",
        "Mode": "Combined Ecology Mode 1 (Action Search)",
        **config, 
    ...
    log_results_to_csv(run_log, filename="results_Combined_Mode1.csv")
    ...
    for i, config in enumerate(batches):
        ...
        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"Combined1_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Combined Ecology Mode 1 (Action Search)",
            **config,
            "Best_Rule_Raw": str(best_rule),
            "Best_Rule_SymPy": simp_action,
    ...
    print("\n[✓] Batch complete! Check results_Combined_Mode1.csv and the /figures directory.")