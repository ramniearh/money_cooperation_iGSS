...
MODEL_CONFIG = {
    ...
    "NUM_D": 10,
    "NUM_ROUNDS": 100,
    # --- NEW CENTRAL BANK CONFIGS ---
    "INITIAL_ENDOWMENT": 1,      # Amount given (e.g., 1, 10, 100)
    "ENDOWMENT_FRACTION": 1      # Proportion of population (1.0 = All, 0.5 = Half)
}
...
# Primitive Set tailored for Accumulating Money
...
# =============================================================================
# 3. AGENT-BASED MODEL (Accumulating Money Physics)
# =============================================================================
class CoopAgent(mesa.Agent):
    def __init__(self, model, agent_type="iGSS-Agent"):
        ...
        self.payoff = 0
        self.token = 0 # Defaults to 0; the Model will distribute the wealth.
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        if cooperates:
            ...
            # 2. The Accumulating Monetary Settlement (The "Dirty" Mugging)
            # If recipient has at least 1 token, the environment forces the transfer.
...
def run_evolution():
    ...
    print("Initializing ACCUMULATING MONEY [MODE 1.5] Module...")
    control_fitness = get_control_baseline(MODEL_CONFIG)
    print(f"Control Baseline (Accumulating Capitalist) established at: {control_fitness:.2f} average payoff.")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*60}\n"
        f"      ACCUMULATING MONEY [MODE 1.5] REPORT\n"
        f"{'='*60}\n"
        f"--- CONFIGURATIONS ---\n"
        f"Random Seed: {SEED}\n"
        f"Populations: {model_config['NUM_IGSS']} iGSS | {model_config['NUM_UC']} UC | {model_config['NUM_D']} Defectors\n"
        f"Liquidity (Fiat Genesis): {model_config['INITIAL_ENDOWMENT']} tokens to {int(model_config['ENDOWMENT_FRACTION'] * 100)}% of agents\n\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
    ...
    ax_plot.set_title('Accumulating Money [Mode 1.5]: Evolutionary Dynamics')
