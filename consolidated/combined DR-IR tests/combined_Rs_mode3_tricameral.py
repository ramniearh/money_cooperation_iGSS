...
# High pop and gens to give the 3-Tree engine enough time to find alignment
EVO_CONFIG = {
    "POP_SIZE": 100,        
    "MAX_GENS": 300,         
    "PARSIMONY_TAX": 0.05
...
# =============================================================================
# 2. GENETIC PROGRAMMING SETUP (TRI-CAMERAL MIND)
# =============================================================================
def random_constant():
    return random.choice([-10, -1, 0, 1, 10])

# --- Tree 1: Action Rule (Behavior) ---
...
# --- Tree 2: IR Assessment Rule (Public Ledger) ---
...
# --- Tree 3: DR Assessment Rule (Private Ledger) ---
...
def init_individual(container, func_act, func_ir, func_dr):
    return container([gp.PrimitiveTree(func_act()), gp.PrimitiveTree(func_ir()), gp.PrimitiveTree(func_dr())])

toolbox.register("individual", init_individual, creator.Individual, toolbox.expr_act, toolbox.expr_ir, toolbox.expr_dr)
...
# Custom Genetic Operators for 3-Tree Individuals
...
def mutThreeTrees(individual):
    ...
    if roll < 0.33:
        ...
        individual[1], = gp.mutUniform(individual[1], expr=toolbox.expr_ir, pset=pset_ir)
    else:
        individual[2], = gp.mutUniform(individual[2], expr=toolbox.expr_dr, pset=pset_dr)
    return individual,
...
# =============================================================================
# 3. AGENT-BASED MODEL (Tri-Cameral Resolution)
# =============================================================================
...
class CooperationModel(mesa.Model):
    def __init__(self, rule_act, rule_ir, rule_dr, config=MODEL_CONFIG, control_type=None):
        super().__init__()
...
def evaluate_rules(individual, model_config, evo_config):
    ...
    func_ir = toolbox.compile_ir(expr=individual[1])
    func_dr = toolbox.compile_dr(expr=individual[2])
    
    tot_igss, tot_uc, tot_d = 0, 0, 0
    runs = 3
    for _ in range(runs): 
        m = CooperationModel(rule_act=func_act, rule_ir=func_ir, rule_dr=func_dr, config=model_config)
    ...
    # Combined Parsimony Tax across all 3 trees
...
def get_control_baseline(model_config, control_type):
    ...
    runs = 5
    for _ in range(runs):
        m = CooperationModel(rule_act=None, rule_ir=None, rule_dr=None, config=model_config, control_type=control_type)
        for _ in range(model_config["NUM_ROUNDS"]): m.step()
...
def run_evolution():
    ...
    print("Initializing Combined [MODE 3] (Tri-Cameral Mind Co-Evolution)...")
    ...
    for gen in range(1, EVO_CONFIG["MAX_GENS"] + 1): 
        ...
        if gen % 10 == 0 or gen == 1:
            history["fossil_record"][gen] = f"ACT: {best_in_gen[0]} | IR: {best_in_gen[1]} | DR: {best_in_gen[2]}"
...
def plot_triple_trees(individual):
    ...
    nx.draw_networkx_nodes(g1, p1, ax=ax1, node_size=800, node_color="lightblue")
    nx.draw_networkx_edges(g1, p1, ax=ax1, arrows=False)
    nx.draw_networkx_labels(g1, p1, l1, ax=ax1, font_size=9)
    ax1.set_title("Evolved ACTION Rule", fontsize=12); ax1.axis("off")
    ...
    nx.draw_networkx_nodes(g2, p2, ax=ax2, node_size=800, node_color="lightgreen")
    nx.draw_networkx_edges(g2, p2, ax=ax2, arrows=False)
    nx.draw_networkx_labels(g2, p2, l2, ax=ax2, font_size=9)
    ax2.set_title("Evolved IR Assessment (Public)", fontsize=12); ax2.axis("off")

    # Tree 3: DR
    n3, e3, l3 = gp.graph(individual[2])
    g3 = nx.DiGraph(); g3.add_nodes_from(n3); g3.add_edges_from(e3)
    try: p3 = hierarchy_pos(g3)
    except TypeError: p3 = nx.spring_layout(g3)
    nx.draw_networkx_nodes(g3, p3, ax=ax3, node_size=800, node_color="orange")
    nx.draw_networkx_edges(g3, p3, ax=ax3, arrows=False)
    nx.draw_networkx_labels(g3, p3, l3, ax=ax3, font_size=9)
    ax3.set_title("Evolved DR Assessment (Private)", fontsize=12); ax3.axis("off")
    
    plt.tight_layout(); plt.show(block=False)

def plot_dashboard(best_ind, history, model_config, evo_config, dr_fit, ir_fit):
    raw_act = str(best_ind[0]); raw_ir = str(best_ind[1]); raw_dr = str(best_ind[2])
    simp_act = parse_sympy(raw_act, "ACT"); simp_ir = parse_sympy(raw_ir, "IR"); simp_dr = parse_sympy(raw_dr, "DR")
    ...
    report_text = (
        f"\n{'='*70}\n"
        f"      COMBINED [MODE 3] REPORT (Tri-Cameral Co-Evolution)\n"
        f"{'='*70}\n"
    ...
    ax_plot.set_title('Institutional Competition: Tri-Cameral Mind Co-Evolution')
