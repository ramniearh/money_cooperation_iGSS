...
# Primitive Set explicitly tailored for Indirect Reciprocity
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # Hardcoded Control Baseline: Strict Discriminator (Standing)
        if self.agent_type == "Control-Standing":
            return partner.standing == 1
            
        # iGSS Evaluation: Reads the public standing of the partner
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        if cooperates:
            ...
            donor.standing = 1 # Helping maintains/restores good standing
        else:
            # Defecting against a "Good" agent destroys your standing
            if recipient.standing == 1:
                donor.standing = 0

    def get_fitness_by_type(self):
        fitness_data = {}
        for a_type in ["iGSS-Agent", "Control-Standing", "Unconditional Cooperator", "Defector"]:
...
def get_control_baseline(model_config):
    """Runs a strict Standing population to establish the theoretical optimal baseline."""
    tot_standing = 0
    runs = 5
    for _ in range(runs):
...
def run_evolution():
    ...
    hof = tools.HallOfFame(1) # Elitism tracker
    history = {"max_igss": [], "avg_igss": [], "uc_scores": [], "d_scores": [], "fossil_record": {}}
    
    print("Initializing Indirect Reciprocity Baseline Module...")
    control_fitness = get_control_baseline(MODEL_CONFIG)
    print(f"Control Baseline (Standing) established at: {control_fitness:.2f} average payoff.")
    ...
    for gen in range(1, EVO_CONFIG["MAX_GENS"] + 1): 
        ...
        # Inject Elite into population to prevent regression
...
def simplify_rule(gp_string):
    # Pure math mapping only
    mapping = {
        'add': lambda x, y: x + y, 
        'sub': lambda x, y: x - y, 
        'mul': lambda x, y: x * y,
        'PartnerStanding': sp.Symbol('PartnerStanding')
    }
    try: 
...
def plot_deap_tree(individual):
    """Draws the abstract syntax tree cleanly using a pure python hierarchical layout."""
    nodes, edges, labels = gp.graph(individual)
    g = nx.DiGraph() # Must be directed to identify root
    ...
    try:
        ...
        pos = nx.spring_layout(g, seed=42) # Fallback if graph gets disconnected
    ...
    ax_tree.set_title("GP Rule Abstract Syntax Tree", fontsize=14)
    ax_tree.axis("off")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*50}\n"
        f"      INDIRECT RECIPROCITY (IR) BASELINE REPORT\n"
        f"{'='*50}\n"
        f"--- BEST EVOLVED RULE ---\n"
        f"Raw:   {raw_action}\n"
        f"SymPy: {simp_action}\n\n"
        ...
        f"Control Baseline (Standing): {control_fit:.1f}\n"
    ...
    # Baselines
    ax_plot.axhline(y=control_fit, color='black', linestyle=':', label=f'Control Baseline (Standing): {control_fit:.1f}')
    ax_plot.axhline(y=theoretical_max, color='purple', alpha=0.3, label=f'Maximum Theoretical Fitness: {theoretical_max}')
    
    ax_plot.set_title('Indirect Reciprocity: Evolutionary Performance Dynamics')
