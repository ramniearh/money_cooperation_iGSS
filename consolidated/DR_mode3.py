...
# Boosted for Co-Evolutionary Stability
EVO_CONFIG = {
    "POP_SIZE": 100,        
    "MAX_GENS": 300,         
    "PARSIMONY_TAX": 0.05
...
def parse_sympy(gp_string, rule_type):
    if rule_type == "action":
        mapping = {'add': lambda x,y: x+y, 'sub': lambda x,y: x-y, 'mul': lambda x,y: x*y, 'PartnerInMemory': sp.Symbol('PartnerInMemory')}
    else:
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    report_text = (
        f"\n{'='*60}\n"
        f"      DIRECT RECIPROCITY [MODE 3 - CO-EVOLUTION] REPORT\n"
        f"{'='*60}\n"
        ...
        f"Control Baseline (Pseudocode Logic): {control_fit:.1f}\n"
    ...
    ax_plot.set_title('Direct Reciprocity [Mode 3]: Action & Assessment Co-Evolution')
