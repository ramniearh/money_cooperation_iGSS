...
# =============================================================================
# 2. GENETIC PROGRAMMING SETUP (CO-EVOLUTION)
# =============================================================================
def random_constant():
...
if not hasattr(creator, "FitnessMax"):
    ...
    # Individual is now a LIST containing two separate trees
...
class CooperationModel(mesa.Model):
    def __init__(self, igss_action_rule, igss_assessment_rule, config=MODEL_CONFIG, control_mode=False):
        super().__init__()
...
def evaluate_rules(individual, model_config, evo_config):
    ...
    runs = 3
    for _ in range(runs): 
        m = CooperationModel(igss_action_rule=func_action, igss_assessment_rule=func_assessment, config=model_config)
...
def get_control_baseline(model_config):
    ...
    runs = 5
    for _ in range(runs):
        m = CooperationModel(igss_action_rule=None, igss_assessment_rule=None, config=model_config, control_mode=True)
        for _ in range(model_config["NUM_ROUNDS"]): m.step()
...
def run_evolution():
    ...
    for gen in range(1, EVO_CONFIG["MAX_GENS"] + 1): 
        ...
        if gen % 10 == 0 or gen == 1:
            # Combine fossil record for both trees
...
def parse_sympy(gp_string, rule_type):
    if rule_type == "action":
        mapping = {'add': lambda x,y: x+y, 'sub': lambda x,y: x-y, 'mul': lambda x,y: x*y, 'PartnerStanding': sp.Symbol('PartnerStanding')}
    else:
...
def plot_dual_trees(individual):
    ...
    ax2.set_title("Evolved ASSESSMENT Rule", fontsize=14); ax2.axis("off")
...
def plot_dashboard(best_ind, history, model_config, evo_config, control_fit):
    ...
    simp_act = parse_sympy(raw_act, "action"); simp_ass = parse_sympy(raw_ass, "assessment")
    ...
    report_text = (
        f"\n{'='*60}\n"
        f"      INDIRECT RECIPROCITY [MODE 3 - CO-EVOLUTION] REPORT\n"
        f"{'='*60}\n"
        ...
        f"Control Baseline (Strict Discriminator + Standing): {control_fit:.1f}\n"
    ...
    ax_plot.set_title('Indirect Reciprocity [Mode 3]: Action & Assessment Co-Evolution')
