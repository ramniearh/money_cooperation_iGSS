...
# --- Tree 2: Assessment Rule (Justice System - Strict 2-Variable) ---
...
if not hasattr(creator, "FitnessMax"):
    ...
    # Individual is a LIST containing two separate trees
...
class CoopAgent(mesa.Agent):
    ...
    def evaluate_partner(self, partner):
        ...
        # Hardcoded Control Baseline: Strict Discriminator
        if self.agent_type == "Control-Agent":
            return partner.standing == 1
            
        # EVOLVED iGSS ACTION: Uses Tree 1
        decision_score = self.model.igss_action_rule(partner.standing)
...
class CooperationModel(mesa.Model):
    ...
    def resolve(self, donor, recipient, cooperates):
        ...
        if self.control_mode:
            ...
            decision_score = self.igss_assessment_rule(action_val, recipient.standing)
...
def get_control_baseline(config):
    """Runs a hardcoded Strict Discriminator + Standing Assessment population."""
    tot_control = 0
    runs = 5
    for _ in range(runs):
...
def run_evolution(config):
    ...
    print("Initializing Indirect Reciprocity [MODE 3 - Co-Evolution Search]...")
...
if __name__ == "__main__":
    ...
    # Truth Table Dictionaries specific to IR
    IR_ACTION_DICT = {
    ...
    IR_ASSESSMENT_DICT = {
        (True, True, False, False): "Image Scoring",     
        (True, False, False, True): "Stern Judging",     
        (True, True, False, True): "Simple Standing",    
        (True, False, False, False): "Shunning",         
        (True, True, True, True): "Unconditional Good",
        (False, False, False, False): "Unconditional Bad"
    }
    
    action_vars = ["PartnerStanding"]
    assess_vars = ["DonorAction", "RecipientStanding"]
    ...
    identified_action = IR_ACTION_DICT.get(act_tuple, f"Novel Act: {act_tuple}")
    ...
    identified_assessment = IR_ASSESSMENT_DICT.get(ass_tuple, f"Novel Ass: {ass_tuple}")

    run_log = {
        "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
        "Machine_ID": "Local_VSCode", 
        "Run_ID": f"IR3_TEST_{datetime.now().strftime('%H%M%S')}",
        "Mode": "Indirect Reciprocity Mode 3 (Co-Evolution)",
        **config, 
    ...
    log_results_to_csv(run_log, filename="results_IR_Mode3.csv")
    ...
    for i, config in enumerate(batches):
        ...
        run_log = {
            "Timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M"),
            "Machine_ID": "Local_VSCode", 
            "Run_ID": f"IR3_{config.get('CONFIG_GROUP', 'Default')}_R{config.get('REPETITION', 1)}",
            "Mode": "Indirect Reciprocity Mode 3 (Co-Evolution)",
            **config,
            "Best_Act_Raw": str(best_rule[0]),
            "Best_Act_SymPy": simp_action,
    ...
    print("\n[✓] Batch complete! Check results_IR_Mode3.csv and the /figures directory.")